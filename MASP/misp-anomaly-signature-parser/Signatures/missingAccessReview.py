...
def main():
    ...
    courseAutomatedProcesses = CourseOfAction("Automated Processes", "Automation",
                                              "Implement automated processes to detect missing access review.").getObject()
    ...
    event = Event()
    event.addObjects([signature, policy, role, permission, courseAutomatedProcesses])
    ...
    event.writeToFile('MissingAccessReview')
